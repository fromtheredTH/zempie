...
class MemberScreen extends StatefulWidget {
  MemberScreen({Key? key, required this.community}) : super(key: key);
...
class _MemberScreen extends BaseState<MemberScreen> {
  ...
  Future<List<UserModel>> initUsers() async{
    var response = await DioClient.getCommunityUsers(community.id, 20, 0);
  ...
  Future<void> getUserNextPage() async {
    if (!isLoading && userScrollController.position.extentAfter < 200 && hasUserNextPage) {
      var response = await DioClient.getCommunityUsers(community.id, 20, userPage);
      List<UserModel> userResults = response.data["result"] == null ? [] : response
          .data["result"].map((json) => UserModel.fromJson(json)).toList().cast<
          UserModel>();
      userPage += 1;
